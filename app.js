...
async function handleImport(){
  const input = document.getElementById('supImport');
  const status = document.getElementById('supStatus');
  const files = Array.from(input?.files||[]);
  if(!files.length){ if(status) status.textContent = "Nessun file selezionato."; return; }
  const storeKey = "skf5s:supervisor:data";
  ...
  for (const f of files){
    try{
      ...
      catch(e){ console.error("JSON non valido", e); tot++; continue; }
      const arr = Array.isArray(data) ? data : [data];
      for(const obj of arr){
        const rec = normalizeRecord(obj);
        const idx = current.findIndex(x => x.channel === rec.channel);
        if(idx>=0) current[idx] = rec; else current.push(rec);
        ok++; tot++;
      }
    }catch(e){ console.error("Errore import", e); tot++; }
  }
  localStorage.setItem(storeKey, JSON.stringify(current));
  if(status) status.textContent = `Import riuscito: ${ok}/${tot} record. Linee totali: ${current.length}.`;
  if (typeof renderSupervisor === 'function') try{ renderSupervisor(); }catch(e){}
  if (typeof renderSupervisorWithNotes === 'function') try{ renderSupervisorWithNotes(); }catch(e){}
  if (typeof renderSupervisorChart === 'function') try{ renderSupervisorChart(); }catch(e){}
}

function normalizeRecord(obj){
  const area = obj.area || obj.zona || obj.section || "";
  const channel = obj.channel || obj.canale || obj.linea || obj.CH || obj.ch || "";
  const points = obj.points || obj.voti || {};
  const s1 = points.s1 ?? points["1S"] ?? obj.s1 ?? 0;
  const s2 = points.s2 ?? points["2S"] ?? obj.s2 ?? 0;
  const s3 = points.s3 ?? points["3S"] ?? obj.s3 ?? 0;
  const s4 = points.s4 ?? points["4S"] ?? obj.s4 ?? 0;
  const s5 = points.s5 ?? points["5S"] ?? obj.s5 ?? 0;
  const date = obj.date || obj.data || new Date().toISOString().slice(0,10);
  const delayed = obj.delayed || obj.ritardo || 0;
  const notes = obj.notes || obj.note || [];
  return {area, channel, date, points:{s1,s2,s3,s4,s5}, delayed, notes};
}
