/* ===================== SKF 5S – app.js (v7.10.3) ========================= */
const VERSION='v7.10.3';
...
function updateDashboard(list){
  ...
  $('#kpiAreas').textContent=(list||filteredAreas()).length;
  $('#kpiScore').textContent=pct(score);
...
function drawChart(list){
  const canvas=$('#chartAreas'); const wrap=canvas?.closest('.chart-inner'); const scroller=canvas?.closest('.chart-scroll');
  ...
  const C1=css.getPropertyValue('--c1').trim(),C2=css.getPropertyValue('--c2').trim(),C3=css.getPropertyValue('--c3').trim(),C4=css.getPropertyValue('--c4').trim(),C5=css.getPropertyValue('--c5').trim();
  const secs=ui.sector==='ALL'?['Rettifica','Montaggio']:[ui.sector];
  ...
  if(chartPref.stacked){
    ...
    const MET=6; const groupW=MET*bw+(MET-1)*inner;
    totalW=padL+padR+rows.length*groupW+Math.max(0,(rows.length-1))*gap;
  }

  canvas.style.width=totalW+'px'; canvas.style.height=Hcss+'px';
  canvas.width=Math.round(totalW*(window.devicePixelRatio||1));
  canvas.height=Math.round(Hcss*(window.devicePixelRatio||1));
  const ctx=canvas.getContext('2d'); ctx.setTransform(window.devicePixelRatio||1,0,0,window.devicePixelRatio||1,0,0);
