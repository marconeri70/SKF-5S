...
// Top bar
$('#btnNewArea')?.addEventListener('click',()=>{const line=(prompt('Nuova linea? (es. L3)','L3')||'Lx').trim();if(!line)return;state.areas.push(makeArea(line));save();render();});
...
$('#btnClearFilters')?.addEventListener('click',()=>{ui={q:'',line:'ALL',sector:'ALL',onlyLate:false}; elQ.value=''; elOnlyLate.checked=false; elLineFilter.value='ALL'; setSeg('#btnAll'); render();});
...
function renderArea(area){
  ...
  line.value=area.line||''; line.addEventListener('input',()=>{area.line=line.value.trim(); save(); refreshLineFilter(); buildLineButtons();});
  ...
  sTabs.forEach(t=> t.addEventListener('click',()=>{sTabs.forEach(x=>x.classList.remove('active')); t.classList.add('active'); curS=t.dataset.s; panels.forEach(p=>p.classList.toggle('active',p.dataset.s===curS));}));

  $('.add-item',node).addEventListener('click',()=>{area.sectors[curSector][curS].push({t:"",d:"",p:0,resp:"",due:"",note:""}); save(); refill(); updateScore();});
  ...
  $('.delete-area',node).addEventListener('click',()=>{if(!confirm('Eliminare la linea?'))return; state.areas.splice(state.areas.indexOf(area),1); save(); render();});
...
// Grafico (canvas a mano, nessuna libreria) -----------------------------------
function drawChart(list){
  ...
  // Assi
  const padL=56,padR=16,padT=12,padB=46, plotW=W-padL-padR, plotH=H-padT-padB;
  ...
  const MET=["tot","1S","2S","3S","4S","5S"], COLORS={tot:'#9bb0d6','1S':C1,'2S':C2,'3S':C3,'4S':C4,'5S':C5};
  const bw=14, inner=4, groupW=MET.length*bw+(MET.length-1)*inner, gap=Math.max(18, groupW*.9);
  ...
  rows.forEach(g=>{
    ...
    ctx.save(); ctx.translate(x+groupW/2,padT+plotH+20); ctx.rotate(-Math.PI/12); ctx.fillText(g.line,0,0); ctx.restore();
    x+=groupW+gap;
  });
}
